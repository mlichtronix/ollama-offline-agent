...
function postChat(kind, text, display = true, id = messageId(), attachments = [], replyTo, createdAt = new Date().toISOString(), options = {}) {
  const event = chatStore.append(kind, text, { id, attachments, replyTo, createdAt, ...options });
...
function browserInventory() {
  ...
  return browsers.map(browser => `- ${browser.id}: ${browser.name} (${browser.engine}; ${browser.headless ? 'headless rendering available' : 'listed, but not supported for this renderer'}${browser.id === 'chrome' ? '; preferred for web compatibility' : ''})`).join('\n');
}
async function browserOpen(url, browserId, waitMs) {
  if (!webEnabled()) return 'Web access is disabled. The user can enable it with the Globe button.';
  const target = safeWebUrl(url); const browsers = discoverBrowsers().filter(item => item.headless); const requested = String(browserId || '').trim().toLowerCase(); const preferred = requested ? browsers.find(item => item.id === requested) : undefined;
  if (requested && !preferred) return `No installed browser with id ${requested} can render headlessly. Call list_browsers first.`;
  if (!browsers.length) return 'No installed Chromium-based browser is available. Call list_browsers for detected browsers.';
  const candidates = preferred ? [preferred, ...browsers.filter(item => item.id !== preferred.id)] : browsers; const attempts = [];
...
function excerptLanguage(file) {
  const extension = path.extname(String(file || '')).slice(1).toLowerCase();
  const aliases = { c: 'c', cs: 'csharp', cpp: 'cpp', cxx: 'cpp', h: 'c', hpp: 'cpp', html: 'xml', htm: 'xml', java: 'java', js: 'javascript', jsx: 'javascript', json: 'json', lua: 'lua', md: 'markdown', mjs: 'javascript', py: 'python', ps1: 'powershell', psm1: 'powershell', sh: 'bash', sql: 'sql', ts: 'typescript', tsx: 'typescript', xml: 'xml', yaml: 'yaml', yml: 'yaml' };
  return aliases[extension] || 'plaintext';
}
async function shareFileExcerpt(args) {
  const requestedPath = String(args?.path || '');
  const target = resolveTarget(requestedPath);
  if (!isWorkspaceTarget(target)) return 'File sharing is limited to a non-sensitive file inside the current workspace.';
  if (isAgentInternal(target)) return 'Agent internal state is not available as project context.';
  if (isSensitiveTarget(target)) return 'Blocked by guardrail: sensitive files cannot be shared in chat.';
  const source = await fsp.readFile(target, 'utf8'); const lines = source.split(/\r?\n/);
  const start = Math.max(1, Math.min(lines.length || 1, args?.startLine === undefined ? 1 : Number(args.startLine) || 1));
  const end = Math.max(start, Math.min(lines.length || start, args?.endLine === undefined ? lines.length : Number(args.endLine) || start));
  const excerpt = lines.slice(start - 1, end).join('\n');
  if (end - start + 1 > 500 || Buffer.byteLength(excerpt, 'utf8') > 48000) return `Requested excerpt is too large (${end - start + 1} lines, ${Buffer.byteLength(excerpt, 'utf8')} bytes). Share a smaller range of at most 500 lines and 48 KB.`;
  const relative = path.relative(root(), target);
  postChat('fileExcerpt', excerpt, true, messageId(), [], undefined, new Date().toISOString(), { fileExcerpt: { path: relative, startLine: start, endLine: end, totalLines: lines.length, language: excerptLanguage(relative) } });
  return `Shared ${relative}, lines ${start}-${end} of ${lines.length}, directly in the chat. Do not repeat its raw content in your response; refer to the displayed excerpt.`;
}
...
async function openSharedFileExcerpt(relativePath) {
  try {
    const target = resolveTarget(relativePath);
    if (!isWorkspaceTarget(target) || isAgentInternal(target) || isSensitiveTarget(target)) throw new Error('The shared file is no longer available inside the workspace.');
    const document = await vscode.workspace.openTextDocument(vscode.Uri.file(target));
    await vscode.window.showTextDocument(document, { preview: true });
  } catch (error) { vscode.window.showWarningMessage(`Could not open shared file: ${error.message}`); }
}
...
async function executeToolRaw(call) {
  ...
  try {
    if (call.function.name === 'set_task_plan') {
      const planned = activeTaskRuntime?.setPlan(args.steps);
    ...
    if (call.function.name === 'share_file_excerpt') return await shareFileExcerpt(args);
...
class OfflineChatViewProvider {
  ...
  setup(view) {
    ...
    view._ollamaMessageDisposable = view.webview.onDidReceiveMessage(message => {
      ...
      if (message.type === 'openFileExcerpt' && typeof message.path === 'string') void openSharedFileExcerpt(message.path);
