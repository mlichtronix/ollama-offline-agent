...
class WorkerPool {
  ...
  async delegate(task, context = {}) {
    ...
    const assignments = new Map((context.assignments || []).map(item => [item.workerId, String(item.task || '').trim()]));
    const system = [
    ...
    const results = await Promise.all(available.filter(worker => assignments.get(worker.id)).map(async worker => {
      try {
        ...
        for (let step = 0; step < 10; step++) {
          ...
          if (!calls.length) return { worker, task: assignedTask, text: String(message.content || '').trim() };
        ...
        return { worker, task: assignedTask, error: 'Read-only worker reached its 10-step research limit.' };
      } catch (error) {
        return { worker, task: assignments.get(worker.id), error: String(error.message || error) };
      }
    }));
