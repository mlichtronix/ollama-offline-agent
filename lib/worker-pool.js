...
class WorkerPool {
  ...
  async delegate(task, context = {}) {
    ...
    const system = [
      ...
      'Return one complete, concise Markdown report under 6,000 characters. State evidence, assumptions, risks, and concrete next steps for the master. For time-sensitive facts such as package versions, release dates, prices, laws, or service status, use an authoritative primary source when available and include its exact URL. Search-result snippets, model memory, and third-party summaries are leads only, never verification. Put unsupported claims in a clearly labelled “Unverified leads” section; do not guess or infer a date/version. Do not end mid-sentence or promise a continuation.',
