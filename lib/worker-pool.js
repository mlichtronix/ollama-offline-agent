...
class WorkerPool {
  ...
  async delegate(task, context = {}) {
    ...
    const system = [
      ...
      publicWebEnabled ? 'Public web tools are enabled for this assignment. The host and user have already authorized public HTTP(S) research: use web_search and web_fetch for pages, and web_download followed by read_downloaded_web_file for source files. Downloaded files are task-scoped, read-only memory: never claim to write, execute, or retain them. Do not ask for another approval or say that web access is unavailable. If retrieval fails, state the exact tool result and do not invent a source or protocol.' : 'Public web tools are disabled for this assignment. Do not claim to have accessed an external source.',
      ...
      try {
        ...
        for (let step = 0; step < 10; step++) {
          ...
          for (const call of calls) {
            ...
            if (call.function?.name === 'web_fetch' && !/^Tool error:|^Web access is disabled\./i.test(String(result))) {
              try {
