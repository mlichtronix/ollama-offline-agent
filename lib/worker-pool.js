...
class WorkerPool {
  ...
  async delegate(task, context = {}) {
    ...
    const publicWebEnabled = (context.tools || []).some(tool => ['web_search', 'web_fetch'].includes(tool?.function?.name));
    const system = [
      ...
      publicWebEnabled ? 'Public web tools are enabled for this assignment. The host and user have already authorized public HTTP(S) research: use web_search and web_fetch when the assigned task needs external evidence. Do not ask for another approval or say that web access is unavailable. If retrieval fails, state the exact tool result and do not invent a source or protocol.' : 'Public web tools are disabled for this assignment. Do not claim to have accessed an external source.',
