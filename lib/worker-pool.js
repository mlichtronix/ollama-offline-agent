...
class WorkerPool {
  ...
  async delegate(task, context = {}) {
    ...
    const system = [
      ...
      publicWebEnabled ? 'Public web tools are enabled for this assignment. The host and user have already authorized public HTTP(S) research: use web_search and web_fetch for pages, and web_download followed by read_downloaded_web_file for source files. For a JavaScript-required SPA, call list_browsers then browser_open; it uses a controlled local browser and returns rendered DOM. Downloaded files are task-scoped, read-only memory: never claim to write, execute, or retain them. Do not ask for another approval or say that web access is unavailable. If retrieval fails, state the exact tool result and do not invent a source or protocol.' : 'Public web tools are disabled for this assignment. Do not claim to have accessed an external source.',
