...
const { classifyModelMessage } = require('./model-adapter');
...
class WorkerPool {
  ...
  async delegate(task, context = {}) {
    ...
    const publicWebEnabled = (context.tools || []).some(tool => ['web_search', 'web_fetch', 'web_download', 'browser_open'].includes(tool?.function?.name));
    const system = [
      ...
      publicWebEnabled ? 'Public web tools are enabled for this assignment. The host and user have already authorized public HTTP(S) research: use web_search and web_fetch for pages, and web_download followed by read_downloaded_web_file for source files. For a JavaScript-required SPA, call list_browsers then browser_open; it uses a controlled local browser and returns rendered DOM. Prefer Chrome when it is installed; Edge is a fallback. Downloaded files are task-scoped, read-only memory: never claim to write, execute, or retain them. Do not ask for another approval or say that web access is unavailable. If retrieval fails, state the exact tool result and do not invent a source or protocol.' : 'Public web tools are disabled for this assignment. Do not claim to have accessed an external source.',
      'Use English for all reasoning, tool requests, source notes, and the final report, regardless of the user-facing language. For factual claims, use a source whose authority matches the claim: an official specification or standards body for protocol semantics, official project documentation or registry metadata for package facts, and an official publisher or register for legal/service facts. A vendor blog can support only what that vendor says or does, not universal protocol behavior. Search-result snippets, model memory, and third-party summaries are leads only, never verification. Present architectural tradeoffs as conditional analysis with their assumptions, not universal facts. Return only one JSON object, without Markdown fences or any text outside it: {"summary":"...","findings":[{"claim":"...","confidence":"verified|conditional|unverified","evidence":[{"url":"exact URL retrieved through web_fetch, web_download, or browser_open in this task","note":"why this source supports the claim"}]}],"risks":["..."],"nextSteps":["..."],"unverified":["..."]}. Keep it below 6,000 characters. A claim is verified only if its cited URL was retrieved through one of those host tools during this task; otherwise use conditional or unverified. Do not guess or infer a date/version. Do not end mid-sentence or promise a continuation.'
    ].filter(Boolean).join(' ');
    const results = await Promise.all(available.filter(worker => assignments.get(worker.id)).map(async worker => {
      try {
        const assignment = assignments.get(worker.id); const assignedTask = assignment.task; const fetchedUrls = new Set(); const repairReasons = []; let repairAttempted = false; let invalidOutputAttempted = false;
        ...
        for (let step = 0; step < 10; step++) {
          ...
          const classified = (context.classifyModelMessage || classifyModelMessage)(message, context.tools || []);
          const calls = classified.kind === 'tool_call' ? classified.calls : [];
          if (classified.kind === 'invalid_model_output') {
            if (!invalidOutputAttempted && step < 9) {
              invalidOutputAttempted = true;
              messages.push({ role: 'user', content: `Your output was rejected by the host (${classified.reason}). Do not repeat a tool protocol or plan. Call one supplied tool or return only the required report JSON.` });
              continue;
            }
            return { worker, role: assignment.role, task: assignedTask, requires: assignment.requires, error: `Worker produced invalid model output (${classified.reason}).` };
          }
          if (classified.kind === 'tool_call') messages.push(message);
          if (!calls.length) {
            if (classified.kind === 'empty') {
              return { worker, role: assignment.role, task: assignedTask, requires: assignment.requires, error: 'Worker returned an empty response.' };
            }
          ...
          for (const call of calls) {
            ...
            if (['web_fetch', 'web_download', 'browser_open'].includes(call.function?.name) && !/^Tool error:|^Web access is disabled\./i.test(String(result))) {
              try {
