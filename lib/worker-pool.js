...
class WorkerPool {
  ...
  async health({ benchmark = false } = {}) {
    const workers = this.workers();
    return Promise.all(workers.map(async worker => {
      if (!worker.enabled) return { ...worker, status: 'disabled' };
      try {
        const client = this.client(worker); const [version, models] = await Promise.all([client.version(), client.listModels()]);
        ...
        let profile; let profileError = '';
        try { profile = await client.modelProfile(worker.model); if (benchmark) profile.benchmark = await client.benchmark(worker.model); }
        catch (error) { profileError = String(error.message || error); }
        return { ...worker, status: 'available', version: version.version || '', profile, profileError };
      } catch (error) {
      ...
      try {
        ...
        for (let step = 0; step < 10; step++) {
          ...
          if (!calls.length) {
            ...
            return { worker, role: assignment.role, task: assignedTask, text: workerReportMarkdown(report), report, quality: { repairAttempted, repairReasons, accepted: !reasons.length } };
          }
