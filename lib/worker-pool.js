  ...
  async health() {
    ...
    return Promise.all(workers.map(async worker => {
      ...
      try {
        ...
        if (!modelAvailable(models, worker.model)) return { ...worker, status: 'unavailable', version: version.version || '', error: 'Configured model is not installed on this worker endpoint: ' + worker.model };
