...
function normalizeWorkerReport(content, fetchedUrls = new Set()) {
  ...
  const findings = (Array.isArray(value.findings) ? value.findings : []).slice(0, 12).map(item => {
    ...
    let confidence = ['verified', 'conditional', 'unverified'].includes(item?.confidence) ? item.confidence : 'unverified';
    if (confidence === 'verified' && !evidence.some(item => item.fetched)) confidence = 'unverified';
    return { claim: text(item?.claim, 1400), confidence, evidence };
...
class WorkerPool {
  ...
  async delegate(task, context = {}) {
    ...
    const results = await Promise.all(available.filter(worker => assignments.get(worker.id)).map(async worker => {
      try {
        const assignment = assignments.get(worker.id); const assignedTask = assignment.task; const fetchedUrls = new Set();
        ...
        for (let step = 0; step < 10; step++) {
          ...
          if (!calls.length) {
            ...
            return { worker, role: assignment.role, task: assignedTask, text: workerReportMarkdown(report), report };
          }
...
module.exports = { WorkerPool, normalizeWorkers, normalizeWorkerReport, workerReportMarkdown };
