...
function isToolProtocolPrefix(content) {
  ...
  return /^<\/?(?:tools?|tool_call)\b/i.test(source)
    ...
    || /^\{[\s\S]{0,300}(?:"type"\s*:\s*"function"|"function"\s*:|"name"\s*:\s*"(?:set_task_plan|advance_task_phase|list_files|read_file|web_search|web_fetch|web_download|browser_open)")/i.test(source);
}
