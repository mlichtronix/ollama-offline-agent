...
function exactJsonCalls(content, allowed) {
  try {
    ...
    // Some otherwise capable Ollama templates serialize consecutive top-level
    // JSON call objects instead of filling `message.tool_calls`. Accept this
    // only when the *entire* response is a whitespace-separated JSON sequence
    // and every item is an allowed, structurally valid call.
    const source = text(content); const calls = []; let index = 0;
    while (index < source.length) {
      while (/\s/.test(source[index] || '')) index++;
      if (source[index] !== '{') return [];
      const start = index; let depth = 0; let quoted = false; let escaped = false;
      for (; index < source.length; index++) {
        const character = source[index];
        if (quoted) { if (escaped) escaped = false; else if (character === '\\') escaped = true; else if (character === '"') quoted = false; continue; }
        if (character === '"') { quoted = true; continue; }
        if (character === '{') depth++;
        if (character === '}') { depth--; if (depth === 0) { index++; break; } }
      }
      if (depth !== 0 || quoted) return [];
      let value; try { value = JSON.parse(source.slice(start, index)); } catch { return []; }
      const call = normalizeCall(value, allowed); if (!call) return [];
      calls.push(call);
    }
    return calls;
  }
...
function isToolProtocolPrefix(content) {
  const source = String(content || '').replace(/^\s+/, '');
  // Hold a short JSON prefix while the stream reveals whether it is a
  // compatibility tool call. Ordinary JSON answers are released once the
  // prefix is long enough and has no function-call markers.
  if (source.startsWith('{') && source.length < 256) return true;
  return /^<\/?(?:tools?|tool_call)\b/i.test(source)
    || /^(?:#\s*)?tools?\s*(?:\r?\n|$)/i.test(source)
    || /^you may call one or more functions\b/i.test(source)
    || /^for each function call,? return\b/i.test(source)
    || /^\{[\s\S]{0,500}"name"\s*:\s*"(?:search_chat_history|read_chat_messages|list_files|read_file|share_file_excerpt|search_text|web_search|list_browsers|browser_open|web_fetch|web_download|read_downloaded_web_file|search_downloaded_web_file|write_file|delete_file|rollback_last_change|run_command|git_status|git_diff|git_log|save_skill|set_task_plan|advance_task_phase|declare_verification_blocker)"/i.test(source)
    || /^\{[\s\S]{0,500}(?:"type"\s*:\s*"function"|"function"\s*:)/i.test(source);
}
...
module.exports = { classifyModelMessage, isToolProtocolEcho, isToolProtocolPrefix, isRepetitiveProgressEcho };
