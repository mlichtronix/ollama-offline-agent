...
class TaskRuntime {
  constructor({ mode = 'execute', startedAt = new Date().toISOString() } = {}) {
    this.ui = { mode, state: 'running', startedAt, timeline: [], activity: [], plan: [], workers: { active: 0, total: 0 }, files: [], checks: [], verification: undefined, canRestore: false };
  }
  ...
  setPlan(steps) {
    ...
    const plan = [];
    // A plan describes the complete route from the task's understood state,
    // not merely the transient phase that happened to be visible while the
    // model was composing it. For example Research → Analyze is valid even
    // when the host had already displayed an initial Analyze activity.
    let previous = 'understand';
    ...
    this.ui.plan = plan;
    this.transition(plan[0].phase, 'active', `Plan accepted: ${plan[0].title}`);
  ...
  advance(target, detail = '') {
    ...
    if (next === from) return { ok: false, from, to: next, message: `Task is already in ${next}; perform the next concrete action or advance to the next pending phase.` };
    const currentPlanIndex = (this.ui.plan || []).findIndex(item => item.phase === from && item.status === 'active');
    const nextPlanIndex = (this.ui.plan || []).findIndex(item => item.phase === next);
    if (currentPlanIndex >= 0 && nextPlanIndex >= 0 && nextPlanIndex <= currentPlanIndex) return { ok: false, from, to: next, message: `The accepted plan cannot move backward from ${from} to ${next}. The host reopens an earlier phase only after a concrete verification result.` };
    const expected = (this.ui.plan || []).find(item => item.status === 'pending');
    if (expected && next !== expected.phase) return { ok: false, from, to: next, message: `The accepted plan requires ${expected.phase} next (${expected.title}); cannot advance directly to ${next}.` };
  ...
  reopen(target, detail = '') {
    const next = normalizePhase(target); const index = (this.ui.plan || []).findIndex(item => item.phase === next);
    if (index < 0) return { ok: false, message: `The accepted plan has no ${next} phase to reopen.` };
    for (let position = 0; position < this.ui.plan.length; position++) this.ui.plan[position].status = position < index ? 'complete' : 'pending';
    this.transition(next, 'active', detail || `Host reopened ${next} after a concrete verification requirement.`);
    return { ok: true, ui: this.ui };
  }
  ...
  setVerificationBlocker(reason) {
    const text = String(reason || '').replace(/\s+/g, ' ').trim();
    if (text.length < 12 || text.length > 600) return { ok: false, message: 'A verification blocker must explain the concrete limitation in 12 to 600 characters.' };
    this.ui.verification = { blocker: text, at: new Date().toISOString() };
    return { ok: true, ui: this.ui };
  }
