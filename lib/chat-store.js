...
class ChatStore {
  ...
  append(kind, text, { id = this.createId(), attachments = [], sources = [], replyTo, createdAt = new Date().toISOString() } = {}) {
    const event = { id, kind, text: String(text), createdAt, replyTo, attachments: attachments.map(item => ({ name: item.name, mime: item.mime, path: item.path })), sources: sources.map(item => ({ title: String(item.title || ''), url: String(item.url || ''), favicon: String(item.favicon || '') })).filter(item => item.url) };
