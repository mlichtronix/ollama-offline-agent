...
class ChatStore {
  ...
  append(kind, text, { id = this.createId(), attachments = [], replyTo, createdAt = new Date().toISOString() } = {}) {
  ...
  rememberAssistant(text, id = this.createId(), createdAt) {
    this.conversation.push({ id, role: 'assistant', content: text });
    if (this.conversation.length > 40) this.conversation.shift();
    return this.append('assistant', text, { id, createdAt });
  }
