...
class ChatStore {
  ...
  append(kind, text, { id = this.createId(), attachments = [], sources = [], replyTo, createdAt = new Date().toISOString(), internal = false, fileExcerpt } = {}) {
    const excerpt = fileExcerpt && typeof fileExcerpt === 'object' ? {
      path: String(fileExcerpt.path || '').slice(0, 2000),
      startLine: Math.max(1, Number(fileExcerpt.startLine) || 1),
      endLine: Math.max(1, Number(fileExcerpt.endLine) || 1),
      totalLines: Math.max(0, Number(fileExcerpt.totalLines) || 0),
      language: String(fileExcerpt.language || 'plaintext').slice(0, 80)
    } : undefined;
    const event = { id, kind, text: String(text), createdAt, replyTo, internal: Boolean(internal), attachments: attachments.map(item => ({ name: item.name, mime: item.mime, path: item.path })), sources: sources.map(item => ({ title: String(item.title || ''), url: String(item.url || ''), favicon: String(item.favicon || '') })).filter(item => item.url), ...(excerpt ? { fileExcerpt: excerpt } : {}) };
