...
- Public-web search, download, and JavaScript-rendered page retrieval, explicitly enabled with the **Globe** control. Sources used for an answer are shown below it, retained in local history, and show either their favicon or a globe fallback.
...
Use the **Server-plus** control in the composer to add up to eight Ollama workers. Each worker has a name, an Ollama-compatible HTTP endpoint, a model name, an optional Bearer token, and an enabled state. Tokens are stored per worker in VS Code Secret Storage. The **Autodetect** control is a low-friction setup path: after an explicit click, it scans only active private IPv4 interfaces with at most 254 usable hosts, checks the standard Ollama port (`11434`) and `/api/version`, and adds newly found servers that expose at least one installed model. It never scans public addresses, arbitrary ports, or larger subnets; protected servers that require a Bearer token stay manual. The **Check** action verifies the endpoint and that its configured model is actually installed, without starting a task. A worker is **Available** only when both checks succeed; **Model missing** means the endpoint answered but does not have the selected model; **Unavailable** means the endpoint or authentication check failed. Only available workers can receive a task.

Before a new master task starts, enabled workers are checked in parallel. The planner may skip delegation for a narrow task; otherwise it can select up to every available enabled worker. The master then creates a delegation plan: each selected worker is assigned a distinct expert role and non-overlapping read-only subtask, while the master retains implementation, integration, and verification. Independent assignments run in parallel. A later expert may explicitly depend on an earlier report; the host waits for that report and passes only the relevant handoff to the dependent expert, capped at 12,000 characters total. Cyclic dependency plans are rejected in favor of the safe independent fallback plan. Assignments and results are written to the Output channel. Full worker reports are retained as internal local chat-history records; they do not clutter the chat UI, but the master can retrieve a specific report by its ID when it needs detail beyond the compact handoff.
...
- Search, download, and render public web pages when the shared **Globe** setting is enabled.

Workers cannot write files, run shell commands, change Git state, install software, or access attachments as image pixels. Their final handoff uses a structured schema with findings, confidence, evidence, risks, next steps, and unverified items. For compatible local models, the host requests Ollama JSON output as an additional transport guardrail. If a cloud endpoint rejects structured output, the worker continues with the schema prompt and the host still validates the resulting handoff. The host marks an evidence URL as retrieved only when that worker actually used `web_fetch`, `web_download`, or `browser_open` for it during the task; a report that does not meet the schema is retained as an unverified legacy report. Before handing a result to the master, the host requests one automatic correction for malformed reports, empty handoffs, or claims marked verified without host-retrieved evidence; the correction outcome is recorded in the Output channel. Their findings are returned to the master as research leads; the master must inspect the relevant evidence and performs all edits and tests. Sources must match the claim: standards and protocol semantics require an official specification or standards body, package facts require official project documentation or registry metadata, and legal/service facts require an official publisher or register. A vendor blog can substantiate only the vendor's own claims, not universal technical behavior. For time-sensitive external claims—such as package versions, release dates, prices, laws, or service availability—workers must provide the exact authoritative source URL. Search snippets, model memory, and secondary summaries are not verification; the master must qualify or omit any claim lacking direct support. Architecture tradeoffs are presented as conditional analysis, not universal rules. Worker endpoints therefore receive the task, their assigned subtask, any history or workspace excerpts selected by the worker, and enabled public-web results. Configure only servers you trust.
...
Use the **Globe** icon in the composer to toggle web access. It changes to **Globe-off** when disabled and the preference is stored in VS Code settings. The agent can use `web_search`, `web_fetch`, and task-scoped `web_download` for public HTTP(S) pages while access is enabled. If an installed Chromium browser is available, it can first call `list_browsers` and then use `browser_open` to retrieve a JavaScript-rendered DOM. Browser sessions use a disposable profile, do not write downloads to the workspace, and route HTTP(S) traffic through an extension-host proxy that rejects localhost, LAN, private ranges, and redirect targets resolving to them. Firefox is listed when installed but is not used by the current headless DOM renderer. Localhost, private IP ranges, LAN addresses, `.local` hosts, embedded URL credentials, and non-HTTP(S) schemes are blocked.
