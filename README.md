...
Use the **Server-plus** control in the composer to add up to eight Ollama workers. Each worker has a name, an Ollama-compatible HTTP endpoint, a model name, an optional Bearer token, and an enabled state. Tokens are stored per worker in VS Code Secret Storage. Load the models installed on a worker into the model picker, or enter a custom `model:tag`. The **Check** action verifies the endpoint and that its configured model is actually installed, without starting a task. A worker is **Available** only when both checks succeed; **Model missing** means the endpoint answered but does not have the selected model; **Unavailable** means the endpoint or authentication check failed. Only available workers can receive a task.
