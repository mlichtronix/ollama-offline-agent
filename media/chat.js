...
function renderTaskUi() {
  ...
  const activityMarkup = activity.length ? `<details class="task-activity"${taskUi.state === 'running' ? ' open' : ''}><summary>Activity (${activity.length})</summary><div>${activity.map(item => `<p class="${escapeHtml(item.status)}"><strong>${escapeHtml(labels[item.phase] || item.phase)}</strong><span>${escapeHtml(item.detail)}</span></p>`).join('')}</div></details>` : '';
