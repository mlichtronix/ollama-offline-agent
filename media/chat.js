...
function hasDroppedFiles(event) { return Array.from(event.dataTransfer?.types || []).includes('Files'); }
window.addEventListener('dragenter', event => { if (!hasDroppedFiles(event)) return; event.preventDefault(); composer.classList.add('dragging'); }, true);
window.addEventListener('dragover', event => { if (!hasDroppedFiles(event)) return; event.preventDefault(); composer.classList.add('dragging'); }, true);
window.addEventListener('dragleave', event => { if (!hasDroppedFiles(event) || event.relatedTarget) return; composer.classList.remove('dragging'); }, true);
window.addEventListener('drop', event => { if (!hasDroppedFiles(event)) return; event.preventDefault(); composer.classList.remove('dragging'); attachFiles(event.dataTransfer.files); }, true);
