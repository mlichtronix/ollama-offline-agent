...
function renderTaskUi() {
  ...
  const review = hasReview ? `<section class="task-review"><header><strong>${files.length ? `Edited ${files.length} file${files.length === 1 ? '' : 's'}` : 'Review'}</strong>${files.length ? `<span class="review-summary"><b class="added">+${added}</b><b class="removed">-${removed}</b></span>` : ''}</header>${files.length ? `<ul class="review-files">${files.map(file => `<li><button class="task-file-diff" data-task-diff="${escapeHtml(file.path)}" title="Open diff for ${escapeHtml(file.path)}">${escapeHtml(file.path)}</button><span><b class="added">+${Number(file.added || 0)}</b><b class="removed">-${Number(file.removed || 0)}</b></span></li>`).join('')}</ul>` : ''}${checks.length ? `<div class="review-checks">${checks.map(check => `<span class="${check.passed ? 'passed' : 'failed'}">${check.passed ? '✓' : '×'} ${escapeHtml(check.command)}</span>`).join('')}</div>` : ''}${taskUi.canRestore ? '<button id="restoreCheckpoint" class="secondary">Restore latest checkpoint</button>' : ''}</section>` : '';
...
function persistScroll() { vscode.setState({ ...vscode.getState(), stickToBottom, scrollTop: chat.scrollTop, steeringMode, taskMode, taskUi }); }
