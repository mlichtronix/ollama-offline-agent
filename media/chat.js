...
function renderWorkers(nextWorkers) {
  ...
  const rows = workers.map(worker => {
    ...
    return `<tr class="worker-row ${draftWorkerIds.has(worker.id) ? 'draft' : ''}"><td><input type="checkbox" data-worker-toggle="${escapeHtml(worker.id)}" ${worker.enabled !== false ? 'checked' : ''} title="Use this worker"></td><td><input data-worker-name="${escapeHtml(worker.id)}" value="${escapeHtml(current.name)}" maxlength="80" aria-label="Worker name"></td><td><input data-worker-endpoint="${escapeHtml(worker.id)}" type="url" value="${escapeHtml(current.endpoint)}" aria-label="Worker endpoint"></td><td><div class="worker-model-cell">${select}<button data-load-worker-models="${escapeHtml(worker.id)}" title="Load installed models and check this worker">${refreshSvg}</button></div></td><td><button data-edit-worker-token="${escapeHtml(worker.id)}" class="worker-key ${workerTokenIds.has(worker.id) ? 'saved' : ''}" title="${workerTokenIds.has(worker.id) ? 'Update saved Bearer token' : 'Set Bearer token'}">${keyRoundSvg}</button></td><td><span class="worker-status ${state}" title="${escapeHtml(health?.error || label)}">${draftWorkerIds.has(worker.id) ? draftLabel : label}</span></td><td><button data-worker-remove="${escapeHtml(worker.id)}" title="Remove worker">${trashSvg}</button></td></tr>`;
  }).join('');
  menu.innerHTML = `<p>Workers can read chat and project context, and use enabled public web tools. They cannot write, run commands, or mutate Git.</p><div class="worker-table-wrap"><table class="worker-table"><thead><tr><th>Use</th><th>Name</th><th>Endpoint</th><th>Model</th><th>Bearer</th><th>Status</th><th></th></tr></thead><tbody>${rows || '<tr><td class="menu-empty" colspan="7">Use + to add a worker.</td></tr>'}</tbody></table></div>`;
}
