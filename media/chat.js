...
const receiptInfo = { sent: ['○', 'Sent to agent host'], waiting: ['●', 'Steering waits for the current subtask'], queued: ['○', 'Queued after the current task'], delivered: ['✓', 'Prompt delivered to the model'], read: ['✓✓', 'Prompt read by the model'], superseded: ['–', 'Replaced by a newer steering prompt'] };
function renderReceipt(id) { const receipt = document.querySelector(`[data-receipt-id="${CSS.escape(id)}"]`); if (!receipt) return; const [mark, label] = receiptInfo[promptStates.get(id)] || receiptInfo.sent; receipt.textContent = mark; receipt.title = label; receipt.dataset.state = promptStates.get(id) || 'sent'; }
