...
const workersDialog = document.createElement('div'); workersDialog.id = 'workersDialog'; workersDialog.className = 'worker-dialog'; workersDialog.innerHTML = `<section class="worker-dialog-card" role="dialog" aria-modal="true" aria-labelledby="workersDialogTitle"><header class="worker-dialog-header"><strong id="workersDialogTitle">Read-only workers</strong><div><button id="checkWorkers" title="Check worker availability">Check</button><button id="closeWorkersDialog" class="icon-only" title="Close worker manager" aria-label="Close worker manager">${xSvg}</button></div></header><div id="workersMenu" class="workers-menu"></div></section>`; document.body.appendChild(workersDialog);
...
function renderWorkers(nextWorkers) {
  ...
  const existingDrafts = new Map(workers.map(worker => [worker.id, { name: document.querySelector(`[data-worker-name="${CSS.escape(worker.id)}"]`)?.value ?? worker.name, endpoint: document.querySelector(`[data-worker-endpoint="${CSS.escape(worker.id)}"]`)?.value ?? worker.endpoint, token: document.querySelector(`[data-worker-token="${CSS.escape(worker.id)}"]`)?.value ?? '' }]));
  ...
  const rows = workers.map(worker => {
    const current = existingDrafts.get(worker.id) || worker;
    ...
    const models = workerModels.get(worker.id)?.models || []; const modelOptions = models.map(item => `<option value="${escapeHtml(item.name)}" ${item.name === worker.model ? 'selected' : ''}>${escapeHtml(item.name)}${Number.isFinite(item.size) ? ` · ${item.size} GB` : ''}</option>`).join('');
    const select = models.length ? `<select data-worker-model="${escapeHtml(worker.id)}">${modelOptions}</select>` : `<select disabled title="Loading models from worker"><option>${workerModels.get(worker.id)?.error || 'Loading installed models…'}</option></select>`;
    return `<div class="worker-row"><input type="checkbox" data-worker-toggle="${escapeHtml(worker.id)}" ${worker.enabled !== false ? 'checked' : ''} title="Use this worker"><input data-worker-name="${escapeHtml(worker.id)}" value="${escapeHtml(current.name)}" maxlength="80" aria-label="Worker name"><input data-worker-endpoint="${escapeHtml(worker.id)}" type="url" value="${escapeHtml(current.endpoint)}" aria-label="Worker endpoint">${select}<div class="worker-token"><input type="password" autocomplete="off" data-worker-token="${escapeHtml(worker.id)}" value="${escapeHtml(current.token)}" placeholder="${workerTokenIds.has(worker.id) ? 'Bearer token saved securely — enter replacement' : 'Optional Bearer token'}" aria-label="Worker Bearer token">${clearToken}</div><span class="worker-status ${state}" title="${escapeHtml(health?.error || label)}">${label}</span><button data-worker-remove="${escapeHtml(worker.id)}" title="Remove worker">${trashSvg}</button></div>`;
  }).join('');
  const newModels = workerModels.get('new')?.models || []; const newOptions = newModels.map(item => `<option value="${escapeHtml(item.name)}">${escapeHtml(item.name)}${Number.isFinite(item.size) ? ` · ${item.size} GB` : ''}</option>`).join('');
  const newSelect = newModels.length ? `<select id="workerModel" aria-label="New worker model"><option value="">Select installed model</option>${newOptions}</select>` : `<select id="workerModel" disabled aria-label="New worker model"><option>${workerModels.get('new')?.error || 'Enter endpoint to load models'}</option></select>`;
  menu.innerHTML = `<p>Workers can read chat and project context, and use enabled public web tools. They cannot write, run commands, or mutate Git.</p><div class="worker-table"><div class="worker-table-head"><span>Use</span><span>Name</span><span>Endpoint</span><span>Model</span><span>Bearer</span><span>Status</span><span></span></div><div class="worker-list">${rows || '<span class="menu-empty">No workers configured.</span>'}</div></div><div class="worker-add"><input id="workerName" maxlength="80" value="${escapeHtml(draft.name)}" placeholder="Worker name"><input id="workerEndpoint" type="url" value="${escapeHtml(draft.endpoint)}" placeholder="http://192.168.1.20:11434">${newSelect}<input id="workerToken" type="password" autocomplete="off" value="${escapeHtml(draft.token)}" placeholder="Optional Bearer token"><button id="addWorker">Add worker</button></div>`;
}
...
function scheduleWorkerSave(id) { clearTimeout(workerSaveTimers.get(id)); workerSaveTimers.set(id, setTimeout(() => { const worker = workers.find(item => item.id === id); const name = document.querySelector(`[data-worker-name="${CSS.escape(id)}"]`)?.value.trim(); const endpoint = document.querySelector(`[data-worker-endpoint="${CSS.escape(id)}"]`)?.value.trim(); const token = document.querySelector(`[data-worker-token="${CSS.escape(id)}"]`)?.value || ''; if (!worker || !name || !validWorkerEndpoint(endpoint)) return; worker.name = name; worker.endpoint = endpoint; saveWorkers(); vscode.postMessage({ type: 'probeWorkerModels', id, endpoint, token }); setTimeout(() => vscode.postMessage({ type: 'checkWorkers' }), 350); }, 700)); }
function scheduleNewWorkerProbe() { clearTimeout(workerSaveTimers.get('new')); workerSaveTimers.set('new', setTimeout(() => { const endpoint = document.getElementById('workerEndpoint')?.value.trim(); const token = document.getElementById('workerToken')?.value || ''; if (validWorkerEndpoint(endpoint)) vscode.postMessage({ type: 'probeWorkerModels', id: 'new', endpoint, token }); }, 700)); }
document.addEventListener('click', event => { const addWorker = event.target.closest('#addWorker'); const clearWorkerToken = event.target.closest('[data-clear-worker-token]'); if (clearWorkerToken) { event.preventDefault(); event.stopImmediatePropagation(); vscode.postMessage({ type: 'setWorkerToken', id: clearWorkerToken.dataset.clearWorkerToken, token: '', clearToken: true }); return; } if (!addWorker) return; event.preventDefault(); event.stopImmediatePropagation(); const name = document.getElementById('workerName').value.trim(); const endpoint = document.getElementById('workerEndpoint').value.trim(); const modelName = document.getElementById('workerModel').value.trim(); const token = document.getElementById('workerToken').value.trim(); if (!name || !modelName || !validWorkerEndpoint(endpoint)) return; const id = crypto.randomUUID(); workers.push({ id, name, endpoint, model: modelName, enabled: true }); vscode.postMessage({ type: 'setWorkers', workers, tokens: token ? { [id]: token } : {} }); renderWorkers(); setTimeout(() => vscode.postMessage({ type: 'checkWorkers' }), 350); }, true);
...
document.addEventListener('change', event => { const model = event.target.closest('[data-worker-model]'); if (!model || !model.value.trim()) return; const worker = workers.find(item => item.id === model.dataset.workerModel); if (!worker || worker.model === model.value.trim()) return; worker.model = model.value.trim(); saveWorkers(); setTimeout(() => vscode.postMessage({ type: 'checkWorkers' }), 350); });
...
document.addEventListener('input', event => { const id = event.target.dataset?.workerName || event.target.dataset?.workerEndpoint || event.target.dataset?.workerToken; if (id) { scheduleWorkerSave(id); if (event.target.dataset?.workerToken) setTimeout(() => { const token = document.querySelector(`[data-worker-token="${CSS.escape(id)}"]`); if (token?.value.trim()) vscode.postMessage({ type: 'setWorkerToken', id, token: token.value, clearToken: false }); }, 700); return; } if (event.target.id === 'workerEndpoint' || event.target.id === 'workerToken') scheduleNewWorkerProbe(); });
