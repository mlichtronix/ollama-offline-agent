...
function openWorkersDialog() { renderWorkers(); closeMenus(); workersDialog.classList.add('open'); document.getElementById('closeWorkersDialog').focus(); }
...
function renderWorkers(nextWorkers) {
  ...
  const rows = workers.map(worker => {
    ...
    const identity = editing ? `<div class="worker-edit"><input data-worker-name="${escapeHtml(worker.id)}" value="${escapeHtml(worker.name)}" placeholder="Worker name"><input data-worker-endpoint="${escapeHtml(worker.id)}" type="url" value="${escapeHtml(worker.endpoint)}" placeholder="http://server:11434"></div>` : `<div><strong>${escapeHtml(worker.name)}</strong><small>${escapeHtml(worker.endpoint)}</small></div>`;
    return `<div class="worker-row ${editing ? 'editing' : ''}"><input type="checkbox" data-worker-toggle="${escapeHtml(worker.id)}" ${worker.enabled !== false ? 'checked' : ''} title="Use this worker">${identity}<span class="worker-status ${state}" title="${escapeHtml(health?.error || label)}">${label}</span><button data-edit-worker="${escapeHtml(worker.id)}" title="${editing ? 'Save worker' : 'Edit worker'}">${editing ? checkSvg : pencilSvg}</button><button data-worker-remove="${escapeHtml(worker.id)}" title="Remove worker">${trashSvg}</button><input class="worker-model" data-worker-model="${escapeHtml(worker.id)}" list="workerModels-${escapeHtml(worker.id)}" value="${escapeHtml(worker.model)}" placeholder="model:tag"><datalist id="workerModels-${escapeHtml(worker.id)}">${modelOptions}</datalist><button data-load-worker-models="${escapeHtml(worker.id)}" title="Load models from this worker">${refreshSvg}</button><div class="worker-token"><input type="password" autocomplete="off" data-worker-token="${escapeHtml(worker.id)}" placeholder="${workerTokenIds.has(worker.id) ? 'Bearer token saved securely — enter replacement' : 'Optional Bearer token — stored securely'}">${clearToken}</div></div>`;
  }).join('');
  ...
  menu.innerHTML = `<p>Workers may search chat history, read project files, and use enabled public web tools. They cannot write, run commands, or mutate Git.</p><div class="worker-list">${rows || '<span class="menu-empty">No workers configured.</span>'}</div><div class="worker-add"><input id="workerName" maxlength="80" value="${escapeHtml(draft.name)}" placeholder="Worker name"><input id="workerEndpoint" type="url" value="${escapeHtml(draft.endpoint)}" placeholder="http://192.168.1.20:11434"><div class="new-worker-model"><input id="workerModel" list="newWorkerModels" maxlength="200" value="${escapeHtml(draft.model)}" placeholder="model:tag — select or type"><datalist id="newWorkerModels">${newOptions}</datalist><button id="loadNewWorkerModels" title="Load installed models from this endpoint">${refreshSvg}</button></div><input id="workerToken" type="password" autocomplete="off" value="${escapeHtml(draft.token)}" placeholder="Optional Bearer token — stored securely"><button id="addWorker">Add worker</button></div>`;
}
...
document.addEventListener('click', event => { const existing = event.target.closest('[data-load-worker-models]'); const fresh = event.target.closest('#loadNewWorkerModels'); if (existing) { event.preventDefault(); event.stopImmediatePropagation(); vscode.postMessage({ type: 'loadWorkerModels', id: existing.dataset.loadWorkerModels }); } else if (fresh) { event.preventDefault(); event.stopImmediatePropagation(); vscode.postMessage({ type: 'probeWorkerModels', id: 'new', endpoint: document.getElementById('workerEndpoint').value, token: document.getElementById('workerToken').value }); } }, true);
document.addEventListener('click', event => { const edit = event.target.closest('[data-edit-worker]'); if (!edit) return; event.preventDefault(); event.stopImmediatePropagation(); const id = edit.dataset.editWorker; const worker = workers.find(item => item.id === id); if (!worker) return; if (editingWorkerId !== id) { editingWorkerId = id; renderWorkers(); return; } const name = document.querySelector(`[data-worker-name="${CSS.escape(id)}"]`)?.value.trim(); const endpoint = document.querySelector(`[data-worker-endpoint="${CSS.escape(id)}"]`)?.value.trim(); let parsed; try { parsed = new URL(endpoint); } catch { return; } if (!name || !['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) return; worker.name = name; worker.endpoint = endpoint; editingWorkerId = undefined; saveWorkers(); renderWorkers(); }, true);
...
document.addEventListener('change', event => { const model = event.target.closest('[data-worker-model]'); if (!model || !model.value.trim()) return; const worker = workers.find(item => item.id === model.dataset.workerModel); if (!worker || worker.model === model.value.trim()) return; worker.model = model.value.trim(); saveWorkers(); });
