...
function renderTaskUi() {
  ...
  taskPanel.innerHTML = `<header><div><strong>${escapeHtml(taskUi.mode === 'plan' ? 'Plan' : taskUi.mode === 'ask' ? 'Ask' : 'Task')}</strong><span id="taskDuration" class="task-duration" title="${taskUi.finishedAt ? 'Task duration' : 'Task is running'}"></span></div>${workerMarkup}<span class="task-state">${escapeHtml(taskUi.state || '')}</span></header>${activityMarkup}${review}`;
