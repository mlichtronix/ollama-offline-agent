...
const keyRoundSvg = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21 2a7 7 0 0 1-9.9 9.9L7 16H3v-4l4.1-4.1A7 7 0 0 1 21 2Z"/><path d="m15 6 .01.01"/></svg>';
...
function renderWorkers(nextWorkers) {
  if (Array.isArray(nextWorkers)) { const drafts = workers.filter(worker => draftWorkerIds.has(worker.id)); workers = [...nextWorkers, ...drafts]; }
  const menu = document.getElementById('workersMenu'); if (!menu) return;
  const existingDrafts = new Map(workers.map(worker => [worker.id, { name: document.querySelector(`[data-worker-name="${CSS.escape(worker.id)}"]`)?.value ?? worker.name, endpoint: document.querySelector(`[data-worker-endpoint="${CSS.escape(worker.id)}"]`)?.value ?? worker.endpoint }]));
  const rows = workers.map(worker => {
    ...
    return `<div class="worker-row ${draftWorkerIds.has(worker.id) ? 'draft' : ''}"><input type="checkbox" data-worker-toggle="${escapeHtml(worker.id)}" ${worker.enabled !== false ? 'checked' : ''} title="Use this worker"><input data-worker-name="${escapeHtml(worker.id)}" value="${escapeHtml(current.name)}" maxlength="80" aria-label="Worker name"><input data-worker-endpoint="${escapeHtml(worker.id)}" type="url" value="${escapeHtml(current.endpoint)}" aria-label="Worker endpoint"><div class="worker-model-cell">${select}<button data-load-worker-models="${escapeHtml(worker.id)}" title="Load installed models and check this worker">${refreshSvg}</button></div><button data-edit-worker-token="${escapeHtml(worker.id)}" class="worker-key ${workerTokenIds.has(worker.id) ? 'saved' : ''}" title="${workerTokenIds.has(worker.id) ? 'Update saved Bearer token' : 'Set Bearer token'}">${keyRoundSvg}</button><span class="worker-status ${state}" title="${escapeHtml(health?.error || label)}">${draftWorkerIds.has(worker.id) ? 'Complete row' : label}</span><button data-worker-remove="${escapeHtml(worker.id)}" title="Remove worker">${trashSvg}</button></div>`;
  }).join('');
  menu.innerHTML = `<p>Workers can read chat and project context, and use enabled public web tools. They cannot write, run commands, or mutate Git.</p><div class="worker-table"><div class="worker-table-head"><span>Use</span><span>Name</span><span>Endpoint</span><span>Model</span><span>Bearer</span><span>Status</span><span></span></div><div class="worker-list">${rows || '<span class="menu-empty">Use + to add a worker.</span>'}</div></div>`;
}
...
document.addEventListener('input', event => { const id = event.target.dataset?.workerName || event.target.dataset?.workerEndpoint; if (id) scheduleWorkerSave(id); });
