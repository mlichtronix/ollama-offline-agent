...
function renderTaskUi() {
  ...
  const timeline = taskUi.timeline.map(item => `<li class="${escapeHtml(item.status)}"><span></span><div><strong>${escapeHtml(labels[item.phase] || item.phase)}</strong><small>${escapeHtml(item.detail || '')}</small></div></li>`).join('');
  const files = taskUi.files || []; const checks = taskUi.checks || [];
  const hasReview = files.length || checks.length || taskUi.canRestore;
  const review = hasReview ? `<section class="task-review"><strong>Review</strong>${files.length ? `<div class="review-files">${files.map(file => `<code>${escapeHtml(file.path)}</code>`).join('')}</div>` : ''}${checks.length ? `<div class="review-checks">${checks.map(check => `<span class="${check.passed ? 'passed' : 'failed'}">${check.passed ? '✓' : '×'} ${escapeHtml(check.command)}</span>`).join('')}</div>` : ''}${taskUi.canRestore ? '<button id="restoreCheckpoint" class="secondary">Restore latest checkpoint</button>' : ''}</section>` : '';
