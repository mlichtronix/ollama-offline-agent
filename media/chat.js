...
function attachmentMarkup(items, temporary = false) {
  ...
  const markup = items.map(item => {
    ...
    const remove = temporary ? `<button class="attachment-remove" data-remove-attachment="${escapeHtml(item.clientId)}" title="Cancel attachment">${clearSvg}</button>` : '';
    const content = String(item.mime || '').startsWith('image/') && (item.data || item.preview) ? `<img class="attachment-image" src="${item.preview || `data:${escapeHtml(item.mime)};base64,${item.data}`}" alt="${name}" title="${name}">` : `<span class="attachment-file" title="${name}">${name}</span>`;
    return temporary ? `<span class="queued-attachment">${content}${remove}</span>` : content;
  }).join('');
...
document.addEventListener('click', event => { const model = event.target.closest('[data-model]'); const access = event.target.closest('[data-access]'); const language = event.target.closest('[data-language]'); const clearEndpointToken = event.target.closest('#clearEndpointToken'); const removeAttachment = event.target.closest('[data-remove-attachment]'); if (removeAttachment) { cancelQueuedAttachment(removeAttachment.dataset.removeAttachment); } else if (clearEndpointToken) { vscode.postMessage({ type: 'setEndpoint', endpoint: document.getElementById('endpoint').value, token: '', clearToken: true }); } else if (model) { vscode.postMessage({ type: 'setModel', model: model.dataset.model }); closeMenus(); } else if (access) { vscode.postMessage({ type: 'setAccessMode', mode: access.dataset.access }); closeMenus(); } else if (language) { vscode.postMessage({ type: 'setLanguage', language: language.dataset.language }); closeMenus(); } else if (!event.target.closest('.setting')) closeMenus(); });
