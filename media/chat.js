...
function renderSettings(message) {
  ...
  if (Array.isArray(message.models)) document.getElementById('modelMenu').innerHTML = `<div class="model-controls"><label>Endpoint<div class="endpoint-field"><input id="endpoint" type="url" placeholder="http://127.0.0.1:11434"><span id="endpointStatus" class="endpoint-status ${message.endpointStatus || 'unknown'}" title="${message.endpointStatus === 'connected' ? 'Endpoint reachable' : 'Endpoint unavailable'}">${message.endpointStatus === 'connected' ? checkSvg : banSvg}</span></div></label><label>Bearer token<div class="endpoint-field"><input id="endpointToken" type="password" autocomplete="off" placeholder="Optional — stored securely"><button id="clearEndpointToken" class="clear-token" title="Clear saved token">${clearSvg}</button></div></label><small class="endpoint-note">Saved automatically. LAN and cloud endpoints may receive selected context.</small><div class="generation-controls"><label>Heat <output id="temperatureValue"></output><input id="temperature" type="range" min="0" max="2" step="0.05"></label><label>Context<select id="contextWindow"><option value="0">Auto</option><option value="4096">4K</option><option value="8192">8K</option><option value="16384">16K</option><option value="32768">32K</option><option value="65536">64K</option><option value="131072">128K</option><option value="262144">256K</option></select></label></div><input id="customModel" placeholder="model:tag — Enter downloads or selects"></div><div class="model-list">${message.models.map(model => `<button data-model="${escapeHtml(model.name)}" class="${model.name === message.model ? 'selected' : ''}">${escapeHtml(model.name)} <small>${model.size} GB</small></button>`).join('') || '<span class="menu-empty">No models available from this endpoint</span>'}</div>`;
...
document.addEventListener('click', event => { const model = event.target.closest('[data-model]'); const access = event.target.closest('[data-access]'); const language = event.target.closest('[data-language]'); const clearEndpointToken = event.target.closest('#clearEndpointToken'); if (clearEndpointToken) { vscode.postMessage({ type: 'setEndpoint', endpoint: document.getElementById('endpoint').value, token: '', clearToken: true }); } else if (model) { vscode.postMessage({ type: 'setModel', model: model.dataset.model }); closeMenus(); } else if (access) { vscode.postMessage({ type: 'setAccessMode', mode: access.dataset.access }); closeMenus(); } else if (language) { vscode.postMessage({ type: 'setLanguage', language: language.dataset.language }); closeMenus(); } else if (!event.target.closest('.setting')) closeMenus(); });
let endpointSaveTimer; function saveEndpointWhenReady() { clearTimeout(endpointSaveTimer); endpointSaveTimer = setTimeout(() => { const endpoint = document.getElementById('endpoint'); const token = document.getElementById('endpointToken'); if (!endpoint || !token) return; try { new URL(endpoint.value.trim()); } catch { return; } vscode.postMessage({ type: 'setEndpoint', endpoint: endpoint.value, token: token.value, clearToken: false }); }, 900); }
