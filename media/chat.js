...
function renderTaskUi() {
  ...
  const activeWorkers = Number(taskUi.workers?.active || 0); const workerMarkup = activeWorkers ? `<span class="task-workers" title="${activeWorkers} expert worker${activeWorkers === 1 ? '' : 's'} currently running">${activeWorkers} worker${activeWorkers === 1 ? '' : 's'} running</span>` : '';
