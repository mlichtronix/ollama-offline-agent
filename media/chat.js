...
let taskMode = ['ask', 'plan', 'execute'].includes(restoredState.taskMode) ? restoredState.taskMode : 'execute';
...
const checkCheckSvg = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m18 6-8.5 8.5L6 11"/><path d="m22 6-8.5 8.5"/></svg>';
...
const undoSvg = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 14 4 9l5-5"/><path d="M4 9h10a6 6 0 0 1 6 6v1"/></svg>';
...
function renderTaskUi() {
  ...
  const files = taskUi.files || []; const checks = taskUi.checks || []; const activity = taskUi.activity || []; const plan = taskUi.plan || [];
  const verificationBlocker = taskUi.verification?.blocker;
  const hasReview = files.length || checks.length || verificationBlocker || taskUi.canRestore;
  ...
  const passedChecks = checks.filter(check => check.passed).length; const failedChecks = checks.length - passedChecks;
  ...
  const filesMarkup = files.length ? `<details class="review-files"${files.length <= 5 ? ' open' : ''}><summary><strong>Edited ${files.length} file${files.length === 1 ? '' : 's'}</strong><span class="review-actions">${actions}</span></summary><ul>${files.map(file => `<li${file.deleted ? ' class="deleted"' : ''}><button class="task-file-diff" data-task-diff="${escapeHtml(file.path)}" title="Open ${escapeHtml(file.path)}">${file.deleted ? 'Deleted: ' : ''}${escapeHtml(file.path)}</button><span><b class="added">+${Number(file.added || 0)}</b><b class="removed">-${Number(file.removed || 0)}</b></span></li>`).join('')}</ul></details>` : '';
  const verificationMarkup = verificationBlocker ? `<p class="review-blocker" title="Validation could not be completed automatically"><strong>Verification blocked:</strong> ${escapeHtml(verificationBlocker)}</p>` : '';
  const review = hasReview ? `<section class="task-review">${filesMarkup || (taskUi.canRestore ? `<div class="review-actions">${actions}</div>` : '')}${checksMarkup}${verificationMarkup}</section>` : '';
  const planDone = plan.filter(item => item.status === 'complete').length;
  const planMarkup = plan.length ? `<details class="task-plan"${taskUi.state === 'running' ? ' open' : ''}><summary>Plan (${planDone}/${plan.length})</summary><div>${plan.map(item => `<p class="${escapeHtml(item.status)}"><i aria-hidden="true"></i><strong>${escapeHtml(labels[item.phase] || item.phase)}</strong><span>${escapeHtml(item.title)}</span></p>`).join('')}</div></details>` : '';
  const activityMarkup = activity.length ? `<details class="task-activity"${taskUi.state === 'running' ? ' open' : ''}><summary>Activity (${activity.length})</summary><div>${activity.map(item => `<p class="${escapeHtml(item.status)}"><i aria-hidden="true"></i><strong>${escapeHtml(labels[item.phase] || item.phase)}</strong><span>${escapeHtml(item.detail)}</span></p>`).join('')}</div></details>` : '';
  const activeWorkers = Number(taskUi.workers?.active || 0); const workerMarkup = `<span class="task-workers${activeWorkers ? '' : ' idle'}" title="${activeWorkers ? `${activeWorkers} expert worker${activeWorkers === 1 ? '' : 's'} currently running` : ''}">${activeWorkers ? `${activeWorkers} worker${activeWorkers === 1 ? '' : 's'} running` : 'Workers idle'}</span>`;
  taskPanel.innerHTML = `<header><div><strong>${escapeHtml(taskUi.mode === 'plan' ? 'Plan' : taskUi.mode === 'ask' ? 'Ask' : 'Task')}</strong><span id="taskDuration" class="task-duration" title="${taskUi.finishedAt ? 'Task duration' : 'Task is running'}"></span></div>${workerMarkup}<span class="task-state">${escapeHtml(taskUi.state || '')}</span></header>${planMarkup}${activityMarkup}${review}`;
  document.getElementById('restoreCheckpoint')?.addEventListener('click', event => { event.stopPropagation(); vscode.postMessage({ type: 'restoreCheckpoint' }); });
...
renderTaskUi();
function openWorkersDialog() { renderWorkers(); renderWorkerDiscovery(); closeMenus(); workersDialog.classList.add('open'); document.getElementById('closeWorkersDialog').focus(); for (const worker of workers) vscode.postMessage({ type: 'loadWorkerModels', id: worker.id }); }
function closeWorkersDialog() { workersDialog.classList.remove('open'); document.getElementById('workers').focus(); }
function renderWorkerCheck() { const button = document.getElementById('checkWorkers'); if (!button) return; button.disabled = checkingWorkers; button.setAttribute('aria-busy', String(checkingWorkers)); button.innerHTML = checkingWorkers ? `Check <span class="worker-check-spinner" aria-hidden="true">${refreshSvg}</span>` : 'Check'; }
function renderWorkerDiscovery() { const button = document.getElementById('autoDetectWorkers'); if (!button) return; button.setAttribute('aria-busy', String(discoveringWorkers)); button.innerHTML = discoveringWorkers ? `Cancel <span class="worker-check-spinner" aria-hidden="true">${refreshSvg}</span>` : 'Autodetect'; button.title = discoveringWorkers ? 'Stop local worker autodetect' : 'Find Ollama workers on small local networks'; }
...
const receiptInfo = { delivered: [checkSvg, 'Prompt delivered to the model'], read: [checkCheckSvg, 'Prompt read by the model'] };
function renderReceipt(id) { const receipt = document.querySelector(`[data-receipt-id="${CSS.escape(id)}"]`); if (!receipt) return; const state = promptStates.get(id); const info = receiptInfo[state]; receipt.hidden = !info; receipt.innerHTML = info?.[0] || ''; receipt.title = info?.[1] || ''; receipt.dataset.state = state || 'sent'; }
function setPromptReceipt(id, state) { if (!id) return; promptStates.set(String(id), state); renderReceipt(String(id)); persistScroll(); }
...
function renderFileExcerpt(excerpt, text) {
  const data = excerpt || {}; const path = String(data.path || 'shared file'); const start = Number(data.startLine || 1); const end = Number(data.endLine || start); const total = Number(data.totalLines || 0); const language = normalizeCodeLanguage(data.language || 'plaintext');
  const body = language === 'markdown' ? `<div class="file-excerpt-markdown">${markdown(text)}</div>` : `<pre class="language-${escapeHtml(language || 'plain')}"><code class="hljs${language ? ` language-${escapeHtml(language)}` : ''}">${highlightCode(text, language)}</code></pre>`;
  return `<details class="file-excerpt-card" open><summary><span>${escapeHtml(path)}</span><small>lines ${start}–${end}${total ? ` of ${total}` : ''}</small></summary><div class="file-excerpt-body">${body}</div><button class="file-excerpt-open" data-open-file-excerpt="${escapeHtml(path)}">Open file</button></details>`;
}
function add(kind, text, id, messageAttachments = [], scroll = true, createdAt, messageReplyTo, messageSources = [], fileExcerpt) {
  ...
  element.innerHTML = kind === 'fileExcerpt' ? renderFileExcerpt(fileExcerpt, text) : (kind === 'assistant' || kind === 'user') ? `${kind === 'assistant' ? timestamp : ''}${messageReplyTo?.quote ? `<blockquote class="reply-reference">${escapeHtml(messageReplyTo.quote)}</blockquote>` : ''}${markdown(text)}${kind === 'assistant' ? sourceMarkup(messageAttachments) : ''}${kind === 'user' ? userMetadata : ''}` : '';
  if (kind !== 'assistant' && kind !== 'user' && kind !== 'fileExcerpt') element.textContent = text;
...
function workerRowMarkup(worker) {
  ...
  return `<tr class="worker-row ${draftWorkerIds.has(worker.id) ? 'draft' : ''}" data-worker-row="${escapeHtml(worker.id)}"><td><input type="checkbox" data-worker-toggle="${escapeHtml(worker.id)}" ${worker.enabled !== false ? 'checked' : ''} title="Use this worker"></td><td><input data-worker-name="${escapeHtml(worker.id)}" value="${escapeHtml(worker.name)}" maxlength="80" aria-label="Worker name"></td><td><input data-worker-endpoint="${escapeHtml(worker.id)}" type="url" value="${escapeHtml(worker.endpoint)}" aria-label="Worker endpoint"></td><td><div class="worker-model-cell">${workerModelMarkup(worker)}</div></td><td><button data-load-worker-models="${escapeHtml(worker.id)}" class="worker-refresh" title="Load installed models and check this worker">${refreshSvg}</button></td><td><button data-edit-worker-token="${escapeHtml(worker.id)}" class="worker-key ${workerTokenIds.has(worker.id) ? 'saved' : ''}" title="${workerTokenIds.has(worker.id) ? 'Update saved Bearer token' : 'Set Bearer token'}">${keyRoundSvg}</button></td><td><span class="worker-status ${state}" title="${escapeHtml(health?.error || label)}">${label}</span></td><td><button data-worker-remove="${escapeHtml(worker.id)}" title="Remove worker">${trashSvg}</button></td></tr>`;
}
...
function renderWorkers(nextWorkers) {
  ...
  if (!body || !workerIdsMatch(body)) { menu.innerHTML = `<div class="worker-table-wrap"><table class="worker-table"><colgroup><col class="worker-col-use"><col class="worker-col-name"><col class="worker-col-endpoint"><col class="worker-col-model"><col class="worker-col-refresh"><col class="worker-col-bearer"><col class="worker-col-status"><col class="worker-col-actions"></colgroup><thead><tr><th>Use</th><th>Name</th><th>Endpoint</th><th>Model</th><th></th><th>Bearer</th><th>Status</th><th></th></tr></thead><tbody>${workers.map(workerRowMarkup).join('') || '<tr><td class="menu-empty" colspan="8">Use + to add a worker.</td></tr>'}</tbody></table></div>`; return; }
...
async function steerAgent() { const text = input.value.trim(); if (!text || sending) return; sending = true; try { await Promise.allSettled([...uploads.values()].map(upload => upload.done)); const sentAttachments = queuedAttachments.splice(0); renderQueuedAttachments(); const id = crypto.randomUUID(); add('user', text, id, sentAttachments, true, new Date().toISOString(), replyTo); setPromptReceipt(id, steeringMode === 'queue' ? 'queued' : 'waiting'); input.value = ''; vscode.postMessage({ type: steeringMode === 'queue' ? 'queue' : 'steer', text, id, replyTo, mode: taskMode, attachments: sentAttachments.map(({ name, mime, path }) => ({ name, mime, path })) }); replyTo = undefined; renderReplyContext(); } finally { sending = false; updateSubmit(); } }
...
const taskModeSetting = document.createElement('div'); taskModeSetting.className = 'setting'; taskModeSetting.innerHTML = `<button id="taskMode" class="task-mode" title="Task mode"></button><div id="taskModeMenu" class="setting-menu task-mode-menu"><button data-task-mode="ask">Ask</button><button data-task-mode="plan">Plan</button><button data-task-mode="execute">Execute</button></div>`; steeringSetting.before(taskModeSetting);
...
function renderTaskMode() { const button = document.getElementById('taskMode'); button.textContent = taskMode === 'execute' ? 'Execute' : taskMode[0].toUpperCase() + taskMode.slice(1); button.title = `${button.textContent} mode`; document.querySelectorAll('[data-task-mode]').forEach(item => item.classList.toggle('selected', item.dataset.taskMode === taskMode)); persistScroll(); }
document.getElementById('steeringMode').onclick = event => { event.stopPropagation(); toggleMenu('steeringMenu'); }; renderSteeringMode();
document.getElementById('taskMode').onclick = event => { event.stopPropagation(); toggleMenu('taskModeMenu'); }; renderTaskMode();
document.addEventListener('click', event => { const choice = event.target.closest('[data-task-mode]'); if (!choice) return; taskMode = choice.dataset.taskMode; renderTaskMode(); closeMenus(); });
...
document.getElementById('checkWorkers').addEventListener('click', () => { if (checkingWorkers) return; checkingWorkers = true; renderWorkerCheck(); });
...
chat.addEventListener('click', event => { const copy = event.target.closest('[data-copy-table]'); const file = event.target.closest('[data-open-file-excerpt]'); if (copy) void copyText(copy.dataset.copyTable || ''); if (file) { event.preventDefault(); vscode.postMessage({ type: 'openFileExcerpt', path: file.dataset.openFileExcerpt }); } });
...
window.addEventListener('message', event => { if (event.data?.type !== 'workerHealth') return; checkingWorkers = false; renderWorkerCheck(); });
window.addEventListener('message', event => { if (event.data?.type !== 'workerDiscovery') return; discoveringWorkers = Boolean(event.data.working); renderWorkerDiscovery(); });
window.addEventListener('message', event => { if (event.data?.type !== 'taskUi') return; taskUi = event.data; renderTaskUi(); persistScroll(); });
window.addEventListener('message', event => { if (event.data?.type !== 'taskRestoreResult') return; if (taskUi) { taskUi.canRestore = false; renderTaskUi(); persistScroll(); } });
window.addEventListener('message', event => { if (event.data?.type === 'historyCleared') { promptStates.clear(); persistScroll(); } if (event.data?.type === 'promptState') setPromptReceipt(event.data.id, event.data.state); });
