...
function renderAbout(message) { const menu = document.getElementById('aboutMenu'); menu.innerHTML = `<div class="about-content"><strong>Ollama Offline Coding Agent</strong><span>Version ${escapeHtml(message.version || 'unknown')}</span><p>Offline-first local coding agent for VS Code. Prompts, chat history, resources, and inference stay local unless you deliberately configure a remote endpoint.</p><small>Project history: ${escapeHtml(message.historyPath || '')}</small></div>`; toggleMenu('aboutMenu'); }
...
document.getElementById('about').onclick = event => { event.stopPropagation(); vscode.postMessage({ type: 'about' }); };
