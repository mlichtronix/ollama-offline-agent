...
function renderWorkers(nextWorkers) {
  ...
  const rows = workers.map(worker => {
    ...
    return `<div class="worker-row"><input type="checkbox" data-worker-toggle="${escapeHtml(worker.id)}" ${worker.enabled !== false ? 'checked' : ''} title="Use this worker"><div><strong>${escapeHtml(worker.name)}</strong><small>${escapeHtml(worker.endpoint)}</small></div><span class="worker-status ${state}" title="${escapeHtml(health?.error || label)}">${label}</span><button data-worker-remove="${escapeHtml(worker.id)}" title="Remove worker">${trashSvg}</button><input class="worker-model" data-worker-model="${escapeHtml(worker.id)}" list="workerModels-${escapeHtml(worker.id)}" value="${escapeHtml(worker.model)}" placeholder="model:tag"><datalist id="workerModels-${escapeHtml(worker.id)}">${modelOptions}</datalist><button data-load-worker-models="${escapeHtml(worker.id)}" title="Load models from this worker">${refreshSvg}</button><div class="worker-token"><input type="password" autocomplete="off" data-worker-token="${escapeHtml(worker.id)}" placeholder="${workerTokenIds.has(worker.id) ? 'Bearer token saved securely — enter replacement' : 'Optional Bearer token — stored securely'}">${clearToken}</div></div>`;
  }).join('');
